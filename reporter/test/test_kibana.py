...
class KibanaSourceTestClass(unittest.TestCase):
    ...
    def test_format_kibana_url(self):
        ...
        assert self._source.format_kibana_url('foo') == \
               "https://kibana5.wikia-inc.com/app/kibana#/discover?_g=(time:(from:now-6h,mode:quick,to:now))&_a=(index:'logstash-other-*',query:(query_string:(analyze_wildcard:!t,query:'foo')),sort:!('@timestamp',desc))"

        assert self._source.format_kibana_url('foo bar') == \
               "https://kibana5.wikia-inc.com/app/kibana#/discover?_g=(time:(from:now-6h,mode:quick,to:now))&_a=(index:'logstash-other-*',query:(query_string:(analyze_wildcard:!t,query:'foo%20bar')),sort:!('@timestamp',desc))"
