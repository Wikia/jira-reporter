...
class KibanaSourceTestClass(unittest.TestCase):
    ...
    def test_format_kibana_url(self):
        ...
        assert self._source.format_kibana_url('foo') == \
               "https://kibana.wikia-inc.com/app/kibana#/discover?_g=(time:(from:now-6h,mode:quick,to:now))&_a=(columns:!('@timestamp','@source_host','@message'),index:'logstash-other-*',query:(query_string:(analyze_wildcard:!t,query:'foo')),sort:!('@timestamp',desc))"

        assert self._source.format_kibana_url('foo bar') == \
               "https://kibana.wikia-inc.com/app/kibana#/discover?_g=(time:(from:now-6h,mode:quick,to:now))&_a=(columns:!('@timestamp','@source_host','@message'),index:'logstash-other-*',query:(query_string:(analyze_wildcard:!t,query:'foo%20bar')),sort:!('@timestamp',desc))"

        # TODO
        assert self._source.format_kibana_url('foo', ['@timestamp', 'name']) == \
              "https://kibana.wikia-inc.com/app/kibana#/discover?_g=(time:(from:now-6h,mode:quick,to:now))&_a=(columns:!('@timestamp','name'),index:'logstash-other-*',query:(query_string:(analyze_wildcard:!t,query:'foo')),sort:!('@timestamp',desc))"

        assert self._source.format_kibana_url(
            query=r'@exception.class: "Wikia\Security\Exception" AND @context.transaction: "foo/bar"'
        ) == "https://kibana.wikia-inc.com/app/kibana#/discover?_g=(time:(from:now-6h,mode:quick,to:now))&_a=(columns:!('@timestamp','@source_host','@message'),index:'logstash-other-*',query:(query_string:(analyze_wildcard:!t,query:'%40exception.class%3A%20%22Wikia%5C%5CSecurity%5C%5CException%22%20AND%20%40context.transaction%3A%20%22foo/bar%22')),sort:!('@timestamp',desc))"

        assert self._source.format_kibana_url(
            query=r'@exception.class: "Wikia\Util\AssertionException"'
        ) == "https://kibana.wikia-inc.com/app/kibana#/discover?_g=(time:(from:now-6h,mode:quick,to:now))&_a=(columns:!('@timestamp','@source_host','@message'),index:'logstash-other-*',query:(query_string:(analyze_wildcard:!t,query:'%40exception.class%3A%20%22Wikia%5C%5CUtil%5C%5CAssertionException%22')),sort:!('@timestamp',desc))"
